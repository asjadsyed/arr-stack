...
while [ ! -f /config/jellyseerr-config/settings.json ]; do sleep 1; done
JELLYSEERR_API_KEY=$(jq -r '.main.apiKey' /config/jellyseerr-config/settings.json)
while [ ! -f /config/radarr-config/config.xml ]; do sleep 1; done
RADARR_API_KEY=$(sed -n "s:.*<ApiKey>\\(.*\\)</ApiKey>.*:\\1:p" /config/radarr-config/config.xml)
while [ ! -f /config/sonarr-config/config.xml ]; do sleep 1; done
SONARR_API_KEY=$(sed -n "s:.*<ApiKey>\\(.*\\)</ApiKey>.*:\\1:p" /config/sonarr-config/config.xml)
while [ ! -f /config/bazarr-config/config/config.yaml ] || [ "$(yq -r '.auth.apikey // ""' /config/bazarr-config/config/config.yaml)" = "" ]; do sleep 1; done
BAZARR_API_KEY=$(yq -r '.auth.apikey' /config/bazarr-config/config/config.yaml)
while [ ! -f /config/prowlarr-config/config.xml ]; do sleep 1; done
...
until curl -fsS "$SUBGEN_URL/status" >/dev/null; do sleep 1; done
...
prowlarr_indexer_registered() {
  local indexer_name="$1"
  local implementation="$2"
  local definition_name="$3"

  curl -fsS "$PROWLARR_URL/api/v1/indexer" \
    -H "X-Api-Key: $PROWLARR_API_KEY" \
  | jq -e \
      --arg indexer_name "$indexer_name" \
      --arg implementation "$implementation" \
      --arg definition_name "$definition_name" \
      '
      .[]
      | select(
          .name == $indexer_name
          and .implementation == $implementation
          and .definitionName == $definition_name
        )
      ' >/dev/null \
    && echo true || echo false
}

BITSEARCH_INDEXER_REGISTERED=$(prowlarr_indexer_registered "BitSearch" "Cardigann" "bitsearch")
KICKASSTORRENTS_WS_INDEXER_REGISTERED=$(prowlarr_indexer_registered "kickasstorrents.ws" "Cardigann" "kickasstorrents-ws")
KNABEN_INDEXER_REGISTERED=$(prowlarr_indexer_registered "Knaben" "Knaben" "Knaben")
LIMETORRENTS_INDEXER_REGISTERED=$(prowlarr_indexer_registered "LimeTorrents" "Cardigann" "limetorrents")
...
echo "Knaben indexer already registered: $KNABEN_INDEXER_REGISTERED"
echo "LimeTorrents indexer already registered: $LIMETORRENTS_INDEXER_REGISTERED"
...
if $BITSEARCH_INDEXER_REGISTERED; then
  echo "BitSearch indexer already registered, skipping..."
else
  echo "Registering BitSearch indexer..."
...
if $KICKASSTORRENTS_WS_INDEXER_REGISTERED; then
  echo "kickasstorrents.ws indexer already registered, skipping..."
else
  echo "Registering kickasstorrents.ws indexer..."
  curl -sS "$PROWLARR_URL/api/v1/indexer" \
    -o /dev/null \
    -X POST \
    -H "Content-Type: application/json" \
...
if $KNABEN_INDEXER_REGISTERED; then
  echo "Knaben indexer already registered, skipping..."
else
  echo "Registering Knaben indexer..."
  curl -sS "$PROWLARR_URL/api/v1/indexer" \
    -o /dev/null \
    -X POST \
    -H "Content-Type: application/json" \
...
if $LIMETORRENTS_INDEXER_REGISTERED; then
  echo "LimeTorrents indexer already registered, skipping..."
else
  echo "Registering LimeTorrents indexer..."
  curl -sS "$PROWLARR_URL/api/v1/indexer" \
    -o /dev/null \
    -X POST \
    -H "Content-Type: application/json" \
...
if $TORRENTGALAXYCLONE_INDEXER_REGISTERED; then
  echo "TorrentGalaxyClone indexer already registered, skipping..."
else
  echo "Registering TorrentGalaxyClone indexer..."
  curl -sS "$PROWLARR_URL/api/v1/indexer" \
    -o /dev/null \
    -X POST \
    -H "Content-Type: application/json" \
...
curl -fsS "$BAZARR_URL/api/system/settings" \
  ...
  -F "settings-general-enabled_providers=whisperai" \
  -F "settings-whisperai-endpoint=http://subgen:9000" \
  ...
  -F "settings-general-enabled_providers=supersubtitles" \
;
...
# Deep analyze media file to get audio tracks language.
# Treat unknown language audio track as (changing this will trigger missing subtitles calculation)
# Treat unknown language embedded subtitles track as (changing this will trigger full subtitles indexing using cache)
curl -fsS "$BAZARR_URL/api/system/settings" \
  -H "X-API-KEY: $BAZARR_API_KEY" \
  -F "settings-general-parse_embedded_audio_track=true" \
  -F "settings-general-default_und_audio_lang=en" \
  -F "settings-general-default_und_embedded_subtitles_lang=en" \
;
...
curl -fsS "$BAZARR_URL/api/system/settings" \
  -H "X-API-KEY: $BAZARR_API_KEY" \
  -F "settings-general-use_whisper_fallback=true" \
  -F "settings-general-use_whisper_fallback_series=true" \
;
